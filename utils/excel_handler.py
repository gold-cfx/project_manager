...
from models.project import ProjectStatus, ProjectLevel
    ...
    def generate_project_template(cls, file_path: str) -> bool:
        ...
        try:
            ...
            level_validation = DataValidation(
                type="list", 
                formula1=f'"{",".join([level.value for level in ProjectLevel])}"'
            )
            ...
            status_validation = DataValidation(
                type="list", 
                formula1=f'"{",".join([status.value for status in ProjectStatus])}"'
            )
