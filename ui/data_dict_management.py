...
class DataDictDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.label_edit.setPlaceholderText("如：进行中, 国家级")
        layout.addRow("显示标签*:", self.label_edit)
    ...
    def load_data(self):
        ...
        if self.dict_item:
            ...
            self.label_edit.setText(self.dict_item.dict_label)
    ...
    def get_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "dict_label": self.label_edit.text().strip(),
...
class DataDictManagement(QWidget):
    ...
    def init_ui(self):
        ...
        self.search_edit.setPlaceholderText("搜索键、值或标签...")
        ...
        self.table.setColumnWidth(4, 100)  # 标签
        self.table.setColumnWidth(5, 50)   # 排序
        self.table.setColumnWidth(6, 50)   # 启用
    ...
    def filter_data(self):
        ...
        for item in self.all_data:
            ...
            if search_text and (
                ...
                search_text not in item.dict_value.lower() and
                search_text not in item.dict_label.lower()
            ):
                continue
    ...
    def display_data(self, data: List[DataDict]):
        ...
        for row, item in enumerate(data):
            ...
            self.table.setItem(row, 4, QTableWidgetItem(item.dict_label))
            self.table.setItem(row, 5, QTableWidgetItem(str(item.sort_order)))
            self.table.setItem(row, 6, QTableWidgetItem("是" if item.is_active else "否"))
            self.table.setItem(row, 7, QTableWidgetItem(item.description or ""))
    ...
    def on_delete(self):
        ...
        reply = QMessageBox.question(
            self, "确认删除", 
            f"确定要删除 {item.dict_type} - {item.dict_label} 吗？",
